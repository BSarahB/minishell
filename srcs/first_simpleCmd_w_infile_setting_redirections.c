...
void	ft_outfiles_before_nofile(t_settings *set, t_cmd *cmd)
{
	...
	set->j = 0;
	while (set->j < cmd->simplecmds[set->i]->nb_of_outfile_before_nofile)
	{
...
int	ft_set_fdin_for_first_s_cmd_w_infile(t_settings *set, t_cmd *cmd, \
...
void	ft_fdin_error_msg(t_settings *set, t_cmd *cmd)
{
	if (set->fdin == -1)
	{
		ft_error_msg(cmd->simplecmds[set->i]->infile[set->j]);
...
void	ft_first_simplecmd_w_infile(t_settings *set, t_cmd *cmd)
{
	...
	{
		flag_random_heredoc = ft_set_fdin_for_first_s_cmd_w_infile(set, cmd, \
flag_random_heredoc);
