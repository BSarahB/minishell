...
char	**ft_malloc_infile_tab(t_simpleCmd *simpleCmd)
{
	...
	if (!infile)
		return (NULL);
	simpleCmd->heredoc_track_index = \
		ft_init_ctab(&heredoc_track_index, simpleCmd->nb_of_infile, -1);
	return (infile);
}
...
char	**ft_malloc_outfile_tab(t_simpleCmd *simpleCmd)
{
	...
	if (!outfile)
		return (NULL);
	simpleCmd->append_track_index = ft_init_ctab(&append_track_index, \
												simpleCmd->nb_of_outfile, -1);
	return (outfile);
}
