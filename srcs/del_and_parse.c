...
{
	simpleCmd->heredoc[simpleCmd->k] = ft_dequote(ft_strdup(lst->content));//ft_strdup(lst->content);
	if(simpleCmd->k == simpleCmd->nb_of_heredoc -1)
		simpleCmd->heredoc[simpleCmd->k + 1] = NULL;
	simpleCmd->k = simpleCmd->k + 1;
	cmd->heredocs[cmd->k] = ft_dequote(ft_strdup(lst->content));//ft_strdup(lst->content); //ft_strdup(content);
...
void    ft_parse_redir_in(t_list *lst, t_simpleCmd *simpleCmd, size_t i)
{
	...
	if(simpleCmd->nofile == 0 && lst->title == redir_in)
	{
		if((fdin = open(simpleCmd->infile[i], O_RDONLY)) == -1)
			simpleCmd->nofile = 0;
		else
			close(fdin);
	}
